...
const ProfileTree = (props) => {
  const {
    ...
    edit = false,
    simple_attribs,
    profile_attribs = {},// no_class
    HTag = "h1",
    input_icon = "icon-plus",
  } = props;

  const default_tree = {ids:[],data:{}};
  const init_loading = typeof props.data != "undefined" ? false : true;// load if no external data is provided
  
  let profile_data = { name: "pTree", deep_dive: true, no_class: false, ...profile_attribs };
  ...
  return (
    <>
      <div className={`pTree pTree_wrapper ${name}`}>
        { header ? <div className="pTree_profile">
          <ProfileIcon {...{ ...profile_data, image}} />
          <HTag className="pTree_username">{username}</HTag>
      ...
      {edit ? <div className={`pTree_add ${input_icon} w3-btn w3-card`}
        onClick={() => {
          display_form();
        }}
      >Add Link</div> : null}
